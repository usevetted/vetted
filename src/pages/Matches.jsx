  ...
  const load = async () => {
    ...
    try {
      ...
      all.forEach(match => {
        ...
        if (match.job_title && !g.allJobTitles.includes(match.job_title)) {
          g.allJobTitles.push(match.job_title);
        }
        (match.additional_job_titles || []).forEach(t => {
          if (t && !g.allJobTitles.includes(t)) g.allJobTitles.push(t);
        });
      });
    ...
    <div className="flex-1 flex flex-col min-h-0">
      ...
      <div className="flex-1 flex flex-col min-h-0">
          ...
          {tabs.map(tab => (
            <button
              ...
              className={`text-[13px] font-medium py-2.5 px-0 border-b-2 transition-colors whitespace-nowrap flex-shrink-0 ${
                activeTab === tab.key ? 'text-primary border-primary' : 'text-muted-foreground border-transparent'
              }`}
            >
              {tab.label}
            </button>
        ...
        <div className="flex-1 flex flex-col min-h-0">
          {activeTab === 'matches' && (
            <div className="flex-1 flex flex-col min-h-0">
              {loading && <div className="flex-1 flex items-center justify-center"><LoadingScreen fullscreen={false} /></div>}
              ...
              {!loading && grouped.length > 0 && (
                <div className="flex-1 overflow-y-auto no-scrollbar px-4 pb-6 pt-3 min-h-0">
                  <div className="grid grid-cols-2 gap-3">
                    {grouped.map((group, i) => {
                      const { otherName, otherPicture, otherLinkedin, primaryMatch, allJobTitles } = group;
                      const firstName = otherName?.split(' ')[0] || '';
                      const initials = otherName?.split(' ').map(n => n[0]).join('').slice(0, 2).toUpperCase() || 'U';
                      const extraJobCount = allJobTitles.length - 1;
                          ...
                          <div className="relative aspect-square w-full bg-gradient-to-br from-brand-green-bg to-secondary/40 flex items-center justify-center overflow-hidden">
                            ...
                            {extraJobCount > 0 && (
                              <div className="absolute bottom-2 left-2 bg-primary text-white text-[10px] font-bold px-2 py-0.5 rounded-full">
                                +{extraJobCount} role{extraJobCount > 1 ? 's' : ''}
                              </div>
                            )}
                          </div>
                          <div className="p-3">
